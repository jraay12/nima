...
export function MemberForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      practice_name: defaultMember?.practice_name ?? "",
      practice_email: defaultMember?.practice_email ?? "",
      practice_referral_email: defaultMember?.practice_referral_email ?? "",
      practice_contact_number: defaultMember?.practice_contact_number ?? "",
      fax_number: defaultMember?.fax_number ?? "",
      year: defaultMember?.year ?? "",
      website: defaultMember?.website ?? "",
      city: defaultMember?.city ?? "",
      state: defaultMember?.state ?? "",
      country: defaultMember?.country ?? "",
      is_boardMember: defaultMember?.is_boardMember ?? false,
      board_title: defaultMember?.board_title ?? "",
      speciality: defaultMember?.speciality ?? "",
      member: null,
    },
  });
  ...
  const handleFormSubmit = (data: MemberFormValues) => {
    ...
    if (isEdit) formData.append("id", defaultMember!.id); // tells backend to upsert
    ...
    if (data.member) formData.append("member", data.member);
