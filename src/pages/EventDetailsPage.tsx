...
const EventDetailsPage = () => {
  ...
  const isPast =
    new Date(`${event.month} ${event.day}, ${event.year}`) < new Date();
  ...
  return (
    <div className="bg-[#fafafa] min-h-screen">
          ...
          {event.image ? (
            <div className="relative w-full h-64 md:h-80 rounded-2xl overflow-hidden">
              ...
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-4 py-2.5 rounded-xl text-center shadow-sm">
                ...
                <p className="text-[11px] text-gray-500">{event.year}</p>
              </div>
              {/* Badge over image */}
      ...
      <div className="max-w-5xl mx-auto px-6 mt-8">
        ...
        <FadeIn direction="up" delay={160}>
          <p className="text-xl md:text-2xl font-bold text-gray-700">
            {event.month} {event.day}, {event.year} @ {event.timeRange}
          </p>
        </FadeIn>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <FadeIn direction="up" delay={100}>
          <div className="flex items-center justify-between mt-10 pt-6 border-t border-gray-200 gap-4 flex-wrap">
            {prev ? (
              <button
                onClick={() => navigate(`/events/${i - 1}`)}
                className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-[#027027] transition-colors group"
              >
            ...
            {next ? (
              <button
                onClick={() => navigate(`/events/${i + 1}`)}
                className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-[#027027] transition-colors group ml-auto"
              >
        ...
        {event.featuredSpeaker?.length > 0 && (
          <FadeIn direction="up" delay={180}>
            <div className="mt-12 border-t border-gray-200 pt-10">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
                <span>
                  {event.featuredSpeaker.length > 1
                    ? "Featured Speakers"
                    : "Featured Speaker"}
                </span>
              ...
              <div className="space-y-5">
                {event.featuredSpeaker.map((speaker) => (
                  <div
                    key={speaker.name}
                ...
                scale-x-0 origin-left transition-transform duration-400 group-hover:scale-x-100"
                    ...
                    transition-colors duration-200 group-hover:bg-[#d2eedd]"
                        ...
                        <p className="text-sm text-gray-500 mt-2">
                          {speaker.specialty || "Healthcare Professional"}
                        </p>
        ...
        {event.event_notes && event.event_notes.trim() && (
